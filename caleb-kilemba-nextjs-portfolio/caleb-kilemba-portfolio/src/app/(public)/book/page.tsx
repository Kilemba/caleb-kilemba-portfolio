...
export default async function BookPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string; service?: string }> }) {
  const [settings, services, query] = await Promise.all([getSiteSettings(), prisma.service.findMany({ where: { published: true }, orderBy: { sortOrder: "asc" }, select: { id: true, title: true } }), searchParams]);
