...
export function AdminSidebar() {
  return <aside className="border-b border-[#e1e7ec] bg-[#172033] text-white lg:min-h-screen lg:w-64 lg:border-b-0 lg:border-r"><div className="p-5"><Link href="/admin" className="text-lg font-extrabold">Caleb CMS</Link><nav className="mt-6 grid grid-cols-2 gap-1 sm:grid-cols-3 lg:grid-cols-1">{links.map(([label, href]) => <Link key={href} href={href} className="rounded-lg px-3 py-2 text-sm font-semibold text-slate-200 hover:bg-white/10">{label}</Link>)}</nav><form action={logoutAction} className="mt-6"><button className="w-full rounded-lg border border-white/20 px-3 py-2 text-left text-sm font-semibold hover:bg-white/10">Logout</button></form></div></aside>;
}
