        ...
        <details className="relative lg:hidden">
          ...
          <nav className="absolute right-0 mt-2 w-64 rounded-xl border border-[#e1e7ec] bg-white p-3 shadow-xl" aria-label="Mobile navigation">
            <div className="flex flex-col gap-1">
              {nav.map(([label, href]) => (
                <Link key={href} href={href} className="rounded-lg px-3 py-2 font-semibold hover:bg-[#f7f9fb]">{label}</Link>
              ))}
              {settings.linkedinUrl && (
                <a
                  href={settings.linkedinUrl}
                  target="_blank"
                  rel="noreferrer noopener"
                  className="inline-flex items-center gap-2 rounded-lg px-3 py-2 font-semibold hover:bg-[#f7f9fb]"
                  aria-label={`Connect with ${settings.name} on LinkedIn (opens in a new tab)`}
                >
                  <LinkedInIcon className="h-4 w-4" />
                  LinkedIn
